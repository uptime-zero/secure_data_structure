...
#define COLS 5   // 스택의 개수
#define ROWS 10  // 각 칸의 최대 높이

typedef struct {
    int data[ROWS]; // 구슬에 적힌 번호
...
void push(Stack* stack, int ball) {
    if (isFull(stack)) {
        return;
    }

    stack->data[++(stack->top)] = ball;
}
...
void initStacks(Stack stacks[]) {
    ...
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < COLS; j++) {
            // 0이 아닐 경우에만 push
            if (pinBalls[i][j] != 0) {
                push(&stacks[j], pinBalls[i][j]);   // j 값이 바뀔 때마다 저장되는 스택의 위치가 바뀜
            }
        }
    }
}
...
// 각 칸의 점수 단가 입력
void initScores(Stack stacks[]) {
    for (int i = 0; i < COLS; i++) {
        scanf("%d", &stacks[i].score);
    }
}
...
void processPlayBalls(Stack stacks[]) {
    for (int i = 0; i < 10; i++) {
        ...
        if (peek(&stacks[col]) == ball) {
            ...
            push(&stacks[col], ball);
        }
    }
}

// 최종 점수 계산 및 출력
void printScores(Stack stacks[]) {
    printf("(");
    for (int i = 0; i < COLS; i++) {
        // 구슬 번호 전부 더하기
        int sum = 0;
        ...
        int total = stacks[i].score * sum; // 칸 점수 × 번호 합계
        if (i < COLS - 1) {
            printf("%d점, ", total);
        } else {
            printf("%d점", total);
